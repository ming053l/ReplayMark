"""BasinMark-C: does an exactly-consistent guidance table fix the collapse?"""
import sys, gzip, json, time, itertools
sys.path.insert(0, "/ssd1/ming/basinmark")
import numpy as np, torch
from basinmark.model import BasinModel
from basinmark.carrier import CarrierMark
from basinmark.prng import payload_bits

KEY, MESSAGE = b"basinmark-key-A", 0xA53C7
GEN, PREFIX, NS = 256, 40, 4
GRID = list(itertools.product([8, 16], [0.30, 0.50], [3.0, 10.0], [6.0]))


def c4(tok, n, ntok):
    out = []
    with gzip.open("/ssd1/ming/basinmark/data/c4-validation.json.gz", "rt") as f:
        for line in f:
            ids = tok(json.loads(line)["text"])["input_ids"]
            if len(ids) >= ntok + 60:
                out.append(torch.tensor(ids[:ntok], dtype=torch.long)[None])
                if len(out) == n:
                    return out


def main():
    M = BasinModel()
    drafts = []
    for i, p in enumerate(c4(M.tok, NS, PREFIX)):
        x = M.generate(p, gen_len=GEN, steps=GEN // 2, block_len=32, temperature=0.8,
                       seed=2000 + i).cpu()
        drafts.append((x, np.arange(p.shape[1], p.shape[1] + GEN)))
    print(f"[drafts ready] GEN={GEN}", flush=True)

    for npr, cr, lam, tau in GRID:
        wm = CarrierMark(M, KEY, n_probes=npr, carrier_rate=cr, ctx_rate=0.20,
                         tau=tau, lam=lam, commit_steps=8)
        signs = payload_bits(KEY, npr, MESSAGE)
        pre, post, acc, neg, ch, co = [], [], [], [], [], []
        t0 = time.time()
        for x, span in drafts:
            D0, _ = wm.deltas(x, span)
            neg.append((np.sign(D0) == signs).mean())
            y = wm.embed(x, span, MESSAGE)
            D1, _ = wm.deltas(y, span)
            pre.append((signs * D0).mean()); post.append((signs * D1).mean())
            acc.append((np.sign(D1) == signs).mean())
            ch.append(float((y[0, span] != x[0, span]).float().mean()))
            co.append(wm.last_cost)
        nS = int(round(cr * GEN / npr))
        print(f"M={npr:<3} carrier={cr} |S_j|~{nS:<3} lam={lam:<5} tau={tau} | "
              f"s*Delta {np.mean(pre):+.3f} -> {np.mean(post):+.3f} | "
              f"bit acc {np.mean(acc):.3f} (no-wm {np.mean(neg):.3f}) | "
              f"changed {np.mean(ch):.2f} cost {np.mean(co):.2f} | "
              f"{(time.time()-t0)/NS:.0f}s/sample", flush=True)


if __name__ == "__main__":
    main()
