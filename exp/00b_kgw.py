"""KGW quality-detection curve, same pipeline as everything else.
    ...
    for d in DELTAS:
        ...
        for i, p in enumerate(prompts):
            ...
            y = kgw_generate(M, p, gen_len=GEN, delta=d, key=KEY, temperature=0.8,
                             seed=5000 + i)
            dd = kgw_detect(M, y, span, key=KEY, dedup=True)
            zs.append(dd["z"]); dup.append(dd["dup_frac"])
            zr.append(kgw_detect(M, y, span, key=KEY, dedup=False)["z"])
        ...
        json.dump(rows, open("/ssd1/ming/basinmark/results/kgw.json", "w"), indent=1)
