...
Same protocol as exp/21; fresh offsets (skip=850); outputs saved."""
...
for name, kw in ARMS:
    ...
    for i, p in enumerate(prompts):
        w = ResampleMark(M, KEY, nonce=f"cb-{i}", **BASE, **kw)
        y = w.generate(p, gen_len=GEN, steps=GEN, message=0, seed=9900 + i)
        d = ResampleMark(M, KEY, nonce=f"cb-{i}", **BASE, s_min=0.5,
