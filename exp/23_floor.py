"""Plausibility-floored acceptance: can high R keep dgMARK-level quality?
...
Same protocol as exp/21; fresh offsets (skip=700); outputs saved."""
...
prompts = c4_prompts(M.tok, NS, skip=700)
...
ARMS = [("control", dict(s_min=2.0, retries=1)),
        ("R8k30", dict(s_min=0.5, retries=8, p_floor=0.30)),
        ("R16k30", dict(s_min=0.5, retries=16, p_floor=0.30)),
        ("R8k10", dict(s_min=0.5, retries=8, p_floor=0.10))]
out = {}
for name, kw in ARMS:
    rec, t0 = [], time.time()
    for i, p in enumerate(prompts):
        w = ResampleMark(M, KEY, nonce=f"fl-{i}", **BASE, **kw)
        y = w.generate(p, gen_len=GEN, steps=GEN, message=0, seed=9000 + i)
        d = ResampleMark(M, KEY, nonce=f"fl-{i}", **BASE, s_min=0.5,
