"""Block-local detection under LOCALIZED editing (exp/33b): contiguous-span re-denoise.

exp/33 showed Bonferroni-min loses to pooling under uniformly RANDOM re-denoising (dense
damage, every block hit). This is the sparse counterpart: one contiguous span of 10%/30%
of positions is re-denoised, so most blocks stay intact -- the regime where a per-block
min-statistic should finally win. Same texts, arms, aggregations, and control as exp/33.
...
for arm in ("control", "R16k05"):
    ...
    for i, raw in enumerate(D["ids"][arm]):
        ...
        rec = {}
        for tag, t in (("clean", ids), ("loc10", redenoise(ids, pls[i], 0.10)),
                       ("loc30", redenoise(ids, pls[i], 0.30))):
            rec[tag] = aggregate(det.detect(t, pls[i], GEN, 0))
        rows.append(rec)
        if (i + 1) % 4 == 0:
            print(f"  [{arm}] {i+1}/{len(D['ids'][arm])}", flush=True)
    out[arm] = rows
    for tag in ("clean", "loc10", "loc30"):
...
json.dump(dict(sha=sha, source="29_clean", attack="contiguous", rows=out),
          open("/ssd2/ming/basinmark/results/33b_localized.json", "w"))
