"""Presence-only main detection: every carrier feeds one binomial test.

Half the carriers currently carry payload bits that the main comparison never uses;
sync_frac=1 doubles the presence sample size at zero perplexity cost, which may buy the
detection power that raising R was buying -- and R is what the quality pays for.
Published-protocol PPL over all evaluable outputs, with the valid-generation rate
reported rather than NaN-dropped.
"""
import sys, time
...
KEY, GEN, BLK, NS = b"retrace-key-A", 256, 32, 20
...
def nll1(t):
    ...
    if ids.shape[1] < 8:                     # empty/short generation: report, don't crash
...
nulls = [ResampleMark(M, KEY, retries=1, nonce=f"doc-{i}", **CFG).detect(
    x, pls[i], GEN, 0) for i, x in enumerate(ref)]
print(f"[null] sync rate {np.mean([d['rate_sync'] for d in nulls]):.3f}  "
      f"n_sync {np.mean([d['n_sync'] for d in nulls]):.0f}", flush=True)
...
for R in (1, 2):
    ...
    for i, p in enumerate(prompts):
        ...
        res.append(nll1(M.tok.decode(y[0, pls[i]:pls[i] + GEN], skip_special_tokens=True)))
    ps = np.array(ps)
    ok = [k for k in range(NS) if nr[k][1] >= 50 and res[k][1] >= 50]
    dn = np.array([res[k][0] - nr[k][0] for k in ok])
    print(f"R={R} | sync {np.mean(rates):.3f} | TPR@5% {np.mean(ps<.05):.2f} "
          f"@1% {np.mean(ps<.01):.2f} @0.1% {np.mean(ps<.001):.2f} | "
          f"valid {len(ok)}/{NS} | dNLL median {np.median(dn):+.3f} "
          f"ratio(exp-mean) {np.exp(dn.mean()):.2f} | {(time.time()-t0)/NS:.0f}s/doc",
          flush=True)
